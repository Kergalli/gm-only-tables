...
class GMOnlyTables {
  ...
  static addCheckboxToSheet(app, html, context) {
    ...
    const checkboxHtml = `
      <div class="form-group">
        ...
        <input type="checkbox" id="gm-only-checkbox" name="flags.${this.MODULE_ID}.${this.FLAG_KEY}" ${isGMOnly ? 'checked' : ''}>
      </div>
    `;
    ...
    if (displayFormulaCheckbox.length > 0) {
      ...
      if (formGroup.length > 0) {
        ...
        $html.find('#gm-only-checkbox').off('change').on('change', function() {
          const isChecked = $(this).is(':checked');
          table.setFlag(GMOnlyTables.MODULE_ID, GMOnlyTables.FLAG_KEY, isChecked);
        });
        return;
      }
    }
  ...
  static wrapRollTableRoll() {
    const originalRoll = RollTable.prototype.roll;
    
    RollTable.prototype.roll = async function(options = {}) {
      const isGMOnly = this.getFlag(GMOnlyTables.MODULE_ID, GMOnlyTables.FLAG_KEY);
